...
| P2 `nameof` | 1 | **blocked (probed)** | mcs parses `nameof(x)` as an invocation, but the NRefactory *resolver* yields `UnknownIdentifierResolveResult`. Removal = teach `CSharpResolver`/`ResolveVisitor` to fold nameof invocations to constant strings. Probe hook: `H5_DISABLE_REWRITE=P2`. (Constant interpolated strings containing nameof already fold via S1.) |
| S2/R4 using static + aliases | 1 | **blocked (probed)** | mcs parses `using static` and classic aliases; the resolver resolves aliases but NOT using-static member lookup (`Max` → unknown). Also alias-any-type (C# 12: `using X = int[]` / tuple types) is unparseable by mcs, so alias rewriting must stay regardless. Probe hook: `H5_DISABLE_REWRITE=S2`. Note: R4's directive removal appears redundant (the main visit already strips these directives) — verify and fold R4 into S2 when tackled. |
