...
`float`, `double`), an enum, `T?` and `T[]` of those, a delegate, and another `[ObjectLiteral]` type.
...
class or interface. Only slots are checked — statics, constants, indexers and computed properties
