    Transpose.define("System.String", {
        ...
        statics: {
            ...
            fromCharArray: function (chars, startIndex, length) {
                ...
                length = Transpose.isNumber(length) ? length : chars.length;
            ...
            handleElement: function (provider, index, alignment, formatStr, args) {
                ...
                if (formatStr && value.$boxed && value.type.$kind === "enum") {
                    ...
                    value = value.type.format(Transpose.unbox(value, true), formatStr, provider);
                } else if (formatStr && Transpose.is(value, System.IFormattable)) {
                    value = Transpose.format(Transpose.unbox(value, true), formatStr, provider);
                } if (Transpose.isNumber(value)) {
                    value = Transpose.Int.format(value, formatStr, provider);
                } else if (Transpose.isDate(value)) {
                    value = System.DateTime.format(value, formatStr, provider);
                } else {
                    value = "" + Transpose.toString(value);
                }

                if (alignment) {
                    alignment = parseInt(alignment, 10);

                    if (!Transpose.isNumber(alignment)) {
                        alignment = null;
                    }
                }

                return System.String.alignString(Transpose.toString(value), alignment);
            },
            ...
            compare: function (strA, strB) {
                ...
                if (arguments.length >= 3) {
                    if (!Transpose.isBoolean(arguments[2])) {
                        // StringComparison
                        switch (arguments[2]) {
            ...
            toCharArray: function (str, startIndex, length) {
                ...
                if (!Transpose.hasValue(startIndex)) {
                    startIndex = 0;
                }

                if (!Transpose.hasValue(length)) {
                    length = str.length;
                }
            ...
            split: function (s, strings, limit, options) {
                var re = (!Transpose.hasValue(strings) || strings.length === 0) ? new RegExp("\\s", "g") : new RegExp(strings.map(System.String.escape).join("|"), "g"),
                    res = [],
                    m,
                    i;
            ...
            concat: function (values) {
                ...
                for (var i = 0; i < list.length; i++) {
                    s += list[i] == null ? "" : Transpose.toString(list[i]);
                }
    ...
    Transpose.Class.addExtend(System.String, [System.IComparable$1(System.String), System.IEquatable$1(System.String)]);
