    Transpose.Reflection = {
        ...
        setMetadata: function (type, metadata, ns) {
            if (Transpose.isString(type)) {
                var typeName = type;
                type = Transpose.unroll(typeName);

                if (type == null) {
                    Transpose.Reflection.deferredMeta.push({ typeName: typeName, metadata: metadata, ns: ns });
                    return;
                }
            }
        ...
        initMetaData: function (type, metadata) {
            if (metadata.m) {
                for (var i = 0; i < metadata.m.length; i++) {
                    ...
                    if (m.tprm && Transpose.isArray(m.tprm)) {
                        for (var j = 0; j < m.tprm.length; j++) {
                            m.tprm[j] = Transpose.Reflection.createTypeParam(m.tprm[j], type, m, j);
                        }
                    }
                }
            }
        ...
        getMetadata: function () {
            ...
            if (typeof (metadata) === "function") {
                ...
                if (this.$typeArguments) {
                    ...
                    var arr = Transpose.Reflection.createTypeParams(this.$metadata);
            ...
            if (!this.$initMetaData && metadata) {
                Transpose.Reflection.initMetaData(this, metadata);
            }
        ...
        createTypeParams: function (fn, t) {
            ...
            for (var i = 0; i < args.length; i++) {
                names.push(Transpose.Reflection.createTypeParam(args[i], t, null, i));
            }
        ...
        isGenericType: function (type) {
            return type.$genericTypeDefinition != null || Transpose.Reflection.isGenericTypeDefinition(type);
        },
        ...
        getBaseType: function (type) {
            if (Transpose.isObject(type) || Transpose.Reflection.isInterface(type) || type.prototype == null) {
                return null;
            } else if (Object.getPrototypeOf) {
                return Transpose.Reflection.convertType(Object.getPrototypeOf(type.prototype).constructor);
            } else {
                ...
                if (Object.prototype.hasOwnProperty.call(p, "constructor")) {
                    ...
                    try {
                        ...
                        return Transpose.Reflection.convertType(p.constructor);
                    } finally {
        ...
        getTypeFullName: function (obj) {
            ...
            if (str) {
                var ns = Transpose.Reflection.getTypeNamespace(obj, str);
        ...
        getTypeQName: function (type) {
            return Transpose.Reflection._makeQName(Transpose.Reflection.getTypeFullName(type), type.$assembly);
        },

        getTypeName: function (type) {
            var fullName = Transpose.Reflection.getTypeFullName(type),
        ...
        getTypeNamespace: function (type, name) {
            var fullName = name || Transpose.Reflection.getTypeFullName(type),
                bIndex = fullName.indexOf("["),
                nsIndex = fullName.lastIndexOf(".", bIndex >= 0 ? bIndex : fullName.length),
                ns = nsIndex > 0 ? fullName.substr(0, nsIndex) : "";

            if (type.$assembly) {
                var parentType = Transpose.Reflection._getAssemblyType(type.$assembly, ns);

                if (parentType) {
                    ns = Transpose.Reflection.getTypeNamespace(parentType);
                }
            }
        ...
        getTypeAssembly: function (type) {
            if (type.$isArray) {
                return Transpose.Reflection.getTypeAssembly(type.$elementType);
            }

            if (System.Array.contains([Date, Number, Boolean, String, Function, Array], type)) {
                return Transpose.SystemAssembly;
            }

            return type.$assembly || Transpose.SystemAssembly;
        },
        ...
        _getAssemblyType: function (asm, name) {
            ...
            if (!asm) {
                asm = Transpose.SystemAssembly;
                noAsm = true;
            }

            var rankInfo = Transpose.Reflection._extractArrayRank(name);
            rank = rankInfo.rank;
            name = rankInfo.name;
        ...
        getAssemblyTypes: function (asm) {
            ...
            if (asm.$types) {
                ...
                var traverse = function (s, n) {
                    ...
                    if (typeof (s) === "function" && Transpose.isUpper(n.charCodeAt(0))) {
                        result.push(s);
                    }
                };
        ...
        createAssemblyInstance: function (asm, typeName) {
            var t = Transpose.Reflection.getType(typeName, asm);

            return t ? Transpose.createInstance(t) : null;
        },
        ...
        getInterfaces: function (type) {
            ...
            if (type.$allInterfaces) {
                ...
                return [System.IComparable$1(Transpose.Int), System.IEquatable$1(Transpose.Int), System.IComparable, System.IFormattable];
            ...
            } else if (type === Array || type.$isArray || (t = System.Array._typedArrays[Transpose.getTypeName(type)])) {
        ...
        isInstanceOfType: function (instance, type) {
            return Transpose.is(instance, type);
        },
        ...
        isAssignableFrom: function (baseType, type) {
            ...
            if (baseType === type || Transpose.isObject(baseType)) {
                return true;
            }

            if (Transpose.isFunction(baseType.isAssignableFrom)) {
            ...
            if (Transpose.Reflection.isInterface(baseType) && System.Array.contains(Transpose.Reflection.getInterfaces(type), baseType)) {
                return true;
            }

            if (baseType.$elementType && baseType.$isArray && type.$elementType && type.$isArray) {
                if (Transpose.Reflection.isValueType(baseType.$elementType) !== Transpose.Reflection.isValueType(type.$elementType)) {
                    return false;
                }

                return baseType.$rank === type.$rank && Transpose.Reflection.isAssignableFrom(baseType.$elementType, type.$elementType);
            }
            ...
            if (inheritors) {
                for (i = 0; i < inheritors.length; i++) {
                    r = Transpose.Reflection.isAssignableFrom(baseType, inheritors[i]);
        ...
        isAbstract: function (type) {
            ...
            return ((Transpose.Reflection.getMetaValue(type, "att", 0) & 128) != 0);
        },
        ...
        _getType: function (typeName, asm, re, noinit) {
            ...
            //asm = asm || Transpose.$currentAssembly;
            ...
            if (m) {
                ...
                switch (m[0]) {
                    case "[":
                        ...
                        for (; ;) {
                            next();
                            t = Transpose.Reflection._getType(typeName, null, re);
            ...
            var rankInfo = Transpose.Reflection._extractArrayRank(tname);
            var rank = rankInfo.rank;

            tname = rankInfo.name;

            t = Transpose.Reflection._getAssemblyType(asm, tname);
            ...
            if (!t && noasm) {
                for (var asmName in System.Reflection.Assembly.assemblies) {
                    if (System.Reflection.Assembly.assemblies.hasOwnProperty(asmName) && System.Reflection.Assembly.assemblies[asmName] !== asm) {
                        t = Transpose.Reflection._getType(typeName, System.Reflection.Assembly.assemblies[asmName], null,true);
        ...
        getType: function (typeName, asm) {
            ...
            return typeName ? Transpose.Reflection._getType(typeName, asm) : null;
        },
        ...
        applyConstructor: function (constructor, args) {
            ...
            if (constructor.$$initCtor && constructor.$kind !== "anonymous") {
                var md = Transpose.getMetadata(constructor),
                    count = 0;

                if (md) {
                    var ctors = Transpose.Reflection.getMembers(constructor, 1, 28),
                        found;
                    ...
                    for (var j = 0; j < ctors.length; j++) {
                        ...
                        if (ctor.p && ctor.p.length === args.length) {
                            ...
                            for (var k = 0; k < ctor.p.length; k++) {
                                ...
                                if (!Transpose.is(args[k], p) || args[k] == null && !Transpose.Reflection.canAcceptNull(p)) {
                                    found = false;
                    ...
                    if (Transpose.isFunction(constructor.ctor) && constructor.ctor.length === args.length) {
                        constructor = constructor.ctor;
                    } else {
                        var name = "$ctor",
                            i = 1;

                        while (Transpose.isFunction(constructor[name + i])) {
        ...
        getAttributes: function (type, attrType, inherit) {
            ...
            if (inherit) {
                var b = Transpose.Reflection.getBaseType(type);

                if (b) {
                    a = Transpose.Reflection.getAttributes(b, attrType, true);

                    for (i = 0; i < a.length; i++) {
                        t = Transpose.getType(a[i]);
                        md = Transpose.getMetadata(t);
            ...
            type_md = Transpose.getMetadata(type);

            if (type_md && type_md.at) {
                for (i = 0; i < type_md.at.length; i++) {
                    a = type_md.at[i];

                    if (attrType == null || Transpose.Reflection.isInstanceOfType(a, attrType)) {
                        t = Transpose.getType(a);
                        md = Transpose.getMetadata(t);

                        if (!md || !md.am) {
                            for (var j = result.length - 1; j >= 0; j--) {
                                if (Transpose.Reflection.isInstanceOfType(result[j], t)) {
                                    result.splice(j, 1);
                                }
                            }
                        }
        ...
        getMembers: function (type, memberTypes, bindingAttr, name, params) {
            ...
            if ((bindingAttr & 72) === 72 || (bindingAttr & 6) === 4) {
                ...
                if (b) {
                    result = Transpose.Reflection.getMembers(b, memberTypes & ~1, bindingAttr & (bindingAttr & 64 ? 255 : 247) & (bindingAttr & 2 ? 251 : 255), name, params);
                }
            }
            ...
            var type_md = Transpose.getMetadata(type);
            ...
            if (bindingAttr & 256) {
                while (type) {
                    ...
                    type = Transpose.Reflection.getBaseType(type);
                }
        ...
        createDelegate: function (mi, firstArgument) {
            var isStatic = mi.is || mi.sm,
                ...
                method = Transpose.Reflection.midel(mi, firstArgument, null, bind);
        ...
        midel: function (mi, target, typeArguments, bind) {
            ...
            method = function () {
                ...
                for (var i = 0; i < arguments.length; i++) {
                    ...
                    args[i] = p && p.pt === System.Object ? v : Transpose.unbox(arguments[i]);

                    if (v == null && p && Transpose.Reflection.isValueType(p.pt)) {
                        args[i] = Transpose.getDefaultValue(p.pt);
                    }
                }
            ...
            return bind !== false ? Transpose.fn.bind(target, method) : method;
        },
        ...
        invokeCI: function (ci, args) {
            ...
            if (ci.def) {
                ...
                return Transpose.Reflection.applyConstructor(ci.sn ? ci.td[ci.sn] : ci.td, args);
            }
        },
        ...
        fieldAccess: function (fi, obj) {
            ...
            if (arguments.length === 3) {
                ...
                if (v == null && Transpose.Reflection.isValueType(fi.rt)) {
                    v = Transpose.getDefaultValue(fi.rt);
                }
        ...
        getMetaValue: function (type, name, dv) {
            var md = type.$isTypeParameter ? type : Transpose.getMetadata(type);

            return md ? (md[name] || dv) : dv;
        },

        isArray: function (type) {
            return Transpose.arrayTypes.indexOf(type) >= 0;
        },

        isValueType: function (type) {
            return !Transpose.Reflection.canAcceptNull(type);
        },

        getNestedTypes: function (type, flags) {
            var types = Transpose.Reflection.getMetaValue(type, "nested", []);

            if (flags) {
                var tmp = [];
                for (var i = 0; i < types.length; i++) {
                    var nestedType = types[i],
                        attrs = Transpose.Reflection.getMetaValue(nestedType, "att", 0),
                        access = attrs & 7,
        ...
        getNestedType: function (type, name, flags) {
            var types = Transpose.Reflection.getNestedTypes(type, flags);

            for (var i = 0; i < types.length; i++) {
                if (Transpose.Reflection.getTypeName(types[i]) === name) {
                    return types[i];
                }
            }
        ...
        isGenericMethodDefinition: function (mi) {
            return Transpose.Reflection.isGenericMethod(mi) && !mi.constructed;
        },
    ...
    Transpose.setMetadata = Transpose.Reflection.setMetadata;
